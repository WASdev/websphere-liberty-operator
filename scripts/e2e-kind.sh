...
install_wlo() {
    ...
    kubectl create -f bundle/manifests/liberty.websphere.ibm.com_webspherelibertyapplications.yaml
    kubectl create -f bundle/manifests/liberty.websphere.ibm.com_webspherelibertydumps.yaml
    kubectl create -f bundle/manifests/liberty.websphere.ibm.com_webspherelibertytraces.yaml

    sed -i "s|image: .*|image: ${LOCAL_REGISTRY}/${BUILD_IMAGE}|
            s|namespace: .*|namespace: ${TEST_NAMESPACE}|" internal/deploy/kustomize/daily/base/websphere-liberty-deployment.yaml

    sed -i "s|namespace: .*|namespace: ${TEST_NAMESPACE}|" internal/deploy/kustomize/daily/base/websphere-liberty-roles.yaml
    
    kubectl create -f internal/deploy/kustomize/daily/base/websphere-liberty-deployment.yaml -n ${TEST_NAMESPACE}
    kubectl create -f internal/deploy/kustomize/daily/base/websphere-liberty-roles.yaml -n ${TEST_NAMESPACE}

    # Wait for operator deployment to be ready
    wait_for ${TEST_NAMESPACE} websphere-liberty-controller-manager
}
