...
setup: ## Ensure Operator SDK is installed.
	${COMMON_DIR}/scripts/installers/install-operator-sdk.sh ${OPERATOR_SDK_RELEASE_VERSION}

.PHONY: setup-go
setup-go: ## Ensure Go is installed.
	${COMMON_DIR}/scripts/installers/install-go.sh ${GO_RELEASE_VERSION}
...
kind-e2e-test:
	${COMMON_DIR}/scripts/test/e2e-kind.sh --test-tag "${BUILD_NUMBER}"

build-manifest:
	${COMMON_DIR}/scripts/build/build-manifest.sh --registry "${PUBLISH_REGISTRY}" --image "${OPERATOR_IMAGE}" --tag "${RELEASE_TARGET}"

build-operator-pipeline:
	${COMMON_DIR}/scripts/build/build-operator.sh --registry "${REGISTRY}" --image "${PIPELINE_OPERATOR_IMAGE}" --tag "${RELEASE_TARGET}" --go-version "${GO_VERSION}"

build-manifest-pipeline:
	${COMMON_DIR}/scripts/build/build-manifest.sh --registry "${REGISTRY}" --image "${IMAGE}" --tag "${RELEASE_TARGET}"

build-bundle-pipeline:
	${COMMON_DIR}/scripts/build/build-bundle.sh --prod-image "${PIPELINE_PRODUCTION_IMAGE}" --registry "${REGISTRY}" --image "${PIPELINE_OPERATOR_IMAGE}" --tag "${RELEASE_TARGET}"

build-catalog-pipeline:
	${COMMON_DIR}/scripts/build/build-catalog.sh --prod-image "${OPERATOR_IMAGE}" --registry "${REGISTRY}" --image "${PIPELINE_OPERATOR_IMAGE}" --tag "${RELEASE_TARGET}" --version "${VERSION}"

test-e2e:
	${COMMON_DIR}/scripts/e2e-release.sh --registry-name default-route --registry-namespace openshift-image-registry \
                     --test-tag "${BUILD_NUMBER}" --target "${RELEASE_TARGET}"

test-pipeline-e2e:
	${COMMON_DIR}/scripts/test/e2e-ocp.sh --cluster-url "${CLUSTER_URL}" --cluster-user "${CLUSTER_USER}" --cluster-token "${CLUSTER_TOKEN}" \
