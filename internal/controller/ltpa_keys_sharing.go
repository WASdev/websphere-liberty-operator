...
func (r *ReconcileWebSphereLiberty) reconcileLTPAMetadata(instance *wlv1.WebSphereLibertyApplication, treeMap map[string]interface{}, latestOperandVersion string, assetsFolder *string) (lutils.LeaderTrackerMetadataList, error) {
...
func (r *ReconcileWebSphereLiberty) reconcileLTPAKeys(instance *wlv1.WebSphereLibertyApplication, ltpaKeysMetadata *lutils.LTPAMetadata, ltpaConfigMetadata *lutils.LTPAMetadata) (string, string, string, error) {
	var err error
	ltpaSecretName := ""
...
func (r *ReconcileWebSphereLiberty) generateLTPAKeys(instance *wlv1.WebSphereLibertyApplication, ltpaMetadata *lutils.LTPAMetadata, ltpaConfigMetadata *lutils.LTPAMetadata) (string, string, error) {
	...
	if err != nil && kerrors.IsNotFound(err) {
		...
		if !thisInstanceIsLeader {
			return "", "", fmt.Errorf("Waiting for WebSphereLibertyApplication instance '%s' to generate the shared LTPA keys file for the namespace '%s'.", leaderName, instance.Namespace)
		}
		...
		if err != nil {
			...
			if kerrors.IsNotFound(err) {
				...
				if err != nil {
					return "", "", fmt.Errorf("Failed to create ConfigMap %s", ltpaJobRequest.Name)
				}
			} else {
				return "", "", fmt.Errorf("Failed to get ConfigMap %s", ltpaJobRequest.Name)
			}
		} else {
			...
			if err := r.CreateOrUpdate(ltpaServiceAccount, nil, func() error {
				...
				return "", "", fmt.Errorf("Failed to create ServiceAccount %s", ltpaServiceAccount.Name)
			}
			...
			if err := r.CreateOrUpdate(ltpaRole, nil, func() error {
				...
				return "", "", fmt.Errorf("Failed to create Role %s", ltpaRole.Name)
			}
			if err := r.CreateOrUpdate(ltpaRoleBinding, nil, func() error {
				return nil
			}); err != nil && !kerrors.IsNotFound(err) {
				return "", "", fmt.Errorf("Failed to create RoleBinding %s", ltpaRoleBinding.Name)
			}
			...
			if err == nil {
				...
				if err != nil && kerrors.IsNotFound(err) {
					...
					return "", "", fmt.Errorf("Failed to get Job %s", generateLTPAKeysJob.Name)
				}
			}
		}
		...
		err = r.GetClient().Get(context.TODO(), types.NamespacedName{Name: generateLTPAKeysJob.Name, Namespace: generateLTPAKeysJob.Namespace}, generateLTPAKeysJob)
		if err != nil && kerrors.IsNotFound(err) {
			return "", "", fmt.Errorf("Waiting for the LTPA key to be generated by Job '%s'.", generateLTPAKeysJob.Name)
		} else if err != nil {
			return "", "", fmt.Errorf("Failed to get Job %s", generateLTPAKeysJob.Name)
		}
		if len(generateLTPAKeysJob.Status.Conditions) > 0 && generateLTPAKeysJob.Status.Conditions[0].Type == v1.JobFailed {
			return "", "", fmt.Errorf("Job %s has failed. Manually clean up hung resources by setting .spec.manageLTPA to false in the %s instance.", generateLTPAKeysJob.Name, leaderName)
		}
		return "", "", fmt.Errorf("Waiting for the LTPA key to be generated by Job '%s'.", generateLTPAKeysJob.Name)
	} else if err != nil {
		return "", "", err
...
func (r *ReconcileWebSphereLiberty) generateLTPAConfig(instance *wlv1.WebSphereLibertyApplication, ltpaKeysMetadata *lutils.LTPAMetadata, ltpaConfigMetadata *lutils.LTPAMetadata, passwordEncryptionMetadata *lutils.PasswordEncryptionMetadata, ltpaKeysLastRotation string, lastKeyRelatedRotation string) (string, error) {
	...
	if err != nil {
		...
		// If this instance is not the leader, exit the reconcile loop
		if !thisInstanceIsLeader {
			return ltpaXMLSecret.Name, fmt.Errorf("Waiting for WebSphereLibertyApplication instance '%s' to generate the shared LTPA keys file for the namespace '%s'.", leaderName, instance.Namespace)
		}
	...
	if !thisInstanceIsLeader {
		...
		if err != nil {
			if kerrors.IsNotFound(err) {
				return ltpaXMLSecret.Name, fmt.Errorf("Waiting for WebSphereLibertyApplication instance '%s' to generate the shared LTPA config for the namespace '%s'.", leaderName, instance.Namespace)
			}
			return ltpaXMLSecret.Name, err
		}
	...
	if err != nil && kerrors.IsNotFound(err) {
		...
		// If this instance is not the leader, exit the reconcile loop
		if !thisInstanceIsLeader {
			return ltpaXMLSecret.Name, fmt.Errorf("Waiting for WebSphereLibertyApplication instance '%s' to generate the shared LTPA password Secret for the namespace '%s'.", leaderName, instance.Namespace)
		}
		...
		if _, foundRawPassword := ltpaSecret.Data["rawPassword"]; !foundRawPassword {
			...
			if err != nil {
				...
				if kerrors.IsNotFound(err) {
					...
					if err != nil {
						return ltpaXMLSecret.Name, fmt.Errorf("Failed to create ConfigMap %s", ltpaJobRequest.Name)
					}
				} else {
					return ltpaXMLSecret.Name, fmt.Errorf("Failed to get ConfigMap %s", ltpaJobRequest.Name)
				}
			} else {
				// Create the ServiceAccount
				if err := r.CreateOrUpdate(ltpaServiceAccount, nil, func() error {
					...
					return ltpaXMLSecret.Name, fmt.Errorf("Failed to create ServiceAccount %s", ltpaServiceAccount.Name)
				}
				...
				if err := r.CreateOrUpdate(ltpaRole, nil, func() error {
					...
					return ltpaXMLSecret.Name, fmt.Errorf("Failed to create Role %s", ltpaRole.Name)
				}
				if err := r.CreateOrUpdate(ltpaRoleBinding, nil, func() error {
					return nil
				}); err != nil && !kerrors.IsNotFound(err) {
					return ltpaXMLSecret.Name, fmt.Errorf("Failed to create RoleBinding %s", ltpaRoleBinding.Name)
				}
				...
				if err == nil {
					...
					if err != nil && kerrors.IsNotFound(err) {
						...
						return ltpaXMLSecret.Name, fmt.Errorf("Failed to get Job %s", generateLTPAConfigJob.Name)
					}
				}
			}
			// Reconcile the Job
			err = r.GetClient().Get(context.TODO(), types.NamespacedName{Name: generateLTPAConfigJob.Name, Namespace: generateLTPAConfigJob.Namespace}, generateLTPAConfigJob)
			if err != nil && kerrors.IsNotFound(err) {
				return ltpaXMLSecret.Name, fmt.Errorf("Waiting for the LTPA config to be generated by Job '%s'.", generateLTPAConfigJob.Name)
			} else if err != nil {
				return ltpaXMLSecret.Name, fmt.Errorf("Failed to get Job %s", generateLTPAConfigJob.Name)
			}
			if len(generateLTPAConfigJob.Status.Conditions) > 0 && generateLTPAConfigJob.Status.Conditions[0].Type == v1.JobFailed {
				return ltpaXMLSecret.Name, fmt.Errorf("Job %s has failed. Manually clean up hung resources by setting .spec.manageLTPA to false in the %s instance.", generateLTPAConfigJob.Name, leaderName)
			}
			return ltpaXMLSecret.Name, fmt.Errorf("Waiting for the LTPA config to be generated by Job '%s'.", generateLTPAConfigJob.Name)
		}
	} else if err != nil {
