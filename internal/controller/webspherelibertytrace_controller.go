...
func (r *ReconcileWebSphereLibertyTrace) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	if isInstanceMarkedToBeDeleted {
		if lutils.Contains(instance.GetFinalizers(), traceFinalizer) {
			...
			if err := r.finalizeWebSphereLibertyTrace(reqLogger, instance, prevTraceEnabled, prevPodName, podNamespace); err != nil {
				return reconcile.Result{}, err
			}
...
func (r *ReconcileWebSphereLibertyTrace) finalizeWebSphereLibertyTrace(reqLogger logr.Logger, olt *webspherelibertyv1.WebSphereLibertyTrace, prevTraceEnabled corev1.ConditionStatus, prevPodName string, podNamespace string) error {
