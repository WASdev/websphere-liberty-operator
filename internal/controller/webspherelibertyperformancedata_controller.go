...
// +kubebuilder:rbac:groups=liberty.websphere.ibm.com,resources=webspherelibertyperformancedata;webspherelibertyperformancedata/status;webspherelibertyperformancedata/finalizers,verbs=get;list;watch;create;update;patch;delete,namespace=websphere-liberty-operator
...
// Reconcile reads that state of the cluster for an WebSphereLibertyPerformanceData object and makes changes based on the state read
// and what is in the WebSphereLibertyPerformanceData.Spec
// Note:
// The Controller will requeue the Request to be processed again if the returned error is non-nil or
// Result.Requeue is true, otherwise upon completion it will remove the work from the queue.

func (r *ReconcileWebSphereLibertyPerformanceData) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	reqLogger := r.Log.WithValues("Request.Namespace", request.Namespace, "Request.Name", request.Name)
	reqLogger.Info("Reconciling WebSphereLibertyPerformanceData")

	// Fetch the WebSphereLibertyPerformanceData instance
	instance := &wlv1.WebSphereLibertyPerformanceData{}
	...
	// Check if the WebSphereLibertyPerformanceData instance is marked to be deleted, which is
	...
	if isInstanceMarkedToBeDeleted {
		if utils.Contains(instance.GetFinalizers(), performanceDataFinalizer) {
			...
			if err := r.finalizeWebSphereLibertyPerformanceData(reqLogger, instance); err != nil {
				return reconcile.Result{}, err
			}
...
func isPerformanceDataRunning(instance *wlv1.WebSphereLibertyPerformanceData) bool {
	isStarted := false
	isWorking := false
	isFailed := false
	for _, condition := range instance.Status.Conditions {
		if condition.Type == wlv1.OperationStatusConditionTypeStarted && condition.Status == corev1.ConditionTrue {
			isStarted = true
		}
		if condition.Type == wlv1.OperationStatusConditionTypeCompleted && condition.Status == corev1.ConditionFalse && condition.Reason == "InProgress" {
			isWorking = true
		}
		if condition.Type == wlv1.OperationStatusConditionTypeFailed && condition.Status == corev1.ConditionTrue {
			isFailed = true
		}
	}
...
func (r *ReconcileWebSphereLibertyPerformanceData) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).For(&wlv1.WebSphereLibertyPerformanceData{}, builder.WithPredicates(pred)).WithOptions(controller.Options{
