...
func (r *ReconcileWebSphereLiberty) reconcileSemeruDeployment(wlva *wlv1.WebSphereLibertyApplication, deploy *appsv1.Deployment) {
	...
	requestsMemory := getQuantityFromRequestsOrDefault(instanceResources, corev1.ResourceMemory, "1Gi")
	requestsCPU := getQuantityFromRequestsOrDefault(instanceResources, corev1.ResourceCPU, "500m")
			...
			Containers: []corev1.Container{
				{
					...
					Env: []corev1.EnvVar{
						{Name: "OPENJ9_JAVA_OPTIONS", Value: getOpenJ9JavaOptions(semeruCloudCompiler)},
					},
				},
			},
		},
...
// Builds and returns the OPENJ9_JAVA_OPTIONS string
func getOpenJ9JavaOptions(semeruCloudCompiler *wlv1.WebSphereLibertyApplicationSemeruCloudCompiler) string {
	var sb strings.Builder
	sb.WriteString("-XX:+JITServerLogConnections")
	if semeruCloudCompiler.DisableJIT != nil && *semeruCloudCompiler.DisableJIT {
		sb.WriteString(" -Xnojit")
	}
	return sb.String()
}
